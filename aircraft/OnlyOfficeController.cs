...
public class OnlyOfficeController : ControllerBase
{
    ...
    {
        var backendPublicBaseUrl = ResolveBackendPublicBaseUrl();
    ...
    var currentVersion = document.Versions.SingleOrDefault(version => version.VersionNumber == document.CurrentVersionNumber);
    var fileName = currentVersion?.FileName ?? document.Name;
    var fileType = ResolveFileType(fileName);
    var documentType = ResolveDocumentType(fileType);
    ...
    {
        var document = _catalogService.EnsureDocument(documentId, user: "onlyoffice-file");
        var currentVersion = document.Versions.SingleOrDefault(version => version.VersionNumber == document.CurrentVersionNumber);
        ...
        {
            path = _catalogService.GetSeedOrFallbackFilePath();
        }
    ...
    private static string ResolveFileType(string fileName)
    {
        ...
        {
            "doc" or "docx" or "odt" or "txt" or "rtf" => extension,
            "xls" or "xlsx" or "ods" or "csv" => extension,
            "ppt" or "pptx" or "odp" => extension,
            _ => "docx"
        };
    }
    ...
    {
        return ResolveFileType(fileName) switch
        {
    ...
    private string? ResolveBackendPublicBaseUrl()
    {
        var configuredBaseUrl = _configuration["OnlyOffice:BackendPublicBaseUrl"];
        ...
        if (!IsContainerHostAlias(configuredUri.Host))
        {
            return configuredBaseUrl.TrimEnd('/');
        }

        var hostIpv4 = TryGetHostIpv4Address();
        if (hostIpv4 is null)
        {
            _logger.LogWarning(
                "No se pudo resolver una IPv4 del host para ONLYOFFICE. Se mantiene la URL configurada {BaseUrl}.",
                configuredBaseUrl);
            return configuredBaseUrl.TrimEnd('/');
        }
    ...
    private static bool IsContainerHostAlias(string host)
    {
        return host.Equals("host.containers.internal", StringComparison.OrdinalIgnoreCase)
            || host.Equals("host.docker.internal", StringComparison.OrdinalIgnoreCase);
    }

    private static string? TryGetHostIpv4Address()
    {
        foreach (var networkInterface in NetworkInterface.GetAllNetworkInterfaces())
        {
            ...
            if (!ipProperties.GatewayAddresses.Any(gateway => gateway.Address.AddressFamily == AddressFamily.InterNetwork))
            {
                continue;
            }
            ...
            var address = ipProperties.UnicastAddresses
                ...
                .FirstOrDefault(ipAddress => ipAddress.AddressFamily == AddressFamily.InterNetwork && !ipAddress.Equals(System.Net.IPAddress.Loopback));
